...
/*
	* Create By Alya
	* Whatsapp : https://whatsapp.com/channel/0029Vb0rvI5HwXb6tu2H4H0A
*/

async function GroupUpdate(alya, m, store) {
	if (!m.messageStubType || !m.isGroup) return
	if (global.db?.groups?.[m.chat] && store?.groupMetadata?.[m.chat] && alya.public) {
		...
		if (global.db?.groups?.[m.chat]?.setinfo && messages[m.messageStubType]) {
			await alya.sendMessage(m.chat, { text: `${admin} ${messages[m.messageStubType]}`, mentions: [m.sender, ...(normalizedTarget?.includes('@') ? [`${normalizedTarget}`] : [])]}, { ephemeralExpiration: m.expiration || m?.metadata?.ephemeralDuration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		} else if (m.messageStubType == 20) {
			store.groupMetadata[m.chat] = await alya.groupMetadata(m.chat).catch(e => ({}));
			...
			if (m.fromMe && ((jidNormalizedUser(alya.user.id) == normalizedTarget) || (jidNormalizedUser(alya.user.lid) == normalizedTarget))) {
...
async function GroupParticipantsUpdate(alya, { id, participants, author, action }, store) {
	try {
		...
		if (global.db?.groups?.[id] && store?.groupMetadata?.[id]) {
			...
			for (let n of participants) {
				...
				let profile;
				try {
					profile = await alya.profilePictureUrl(n, 'image');
				} catch {
					profile = 'https://telegra.ph/file/95670d63378f7f4210f03.png';
				}
				let messageText;
				if (action === 'add') {
					if (db.groups[id].welcome) messageText = db.groups[id]?.text?.setwelcome || `Selamat Datang Kak @`;
					...
					if (db.groups[id].leave) messageText = db.groups[id]?.text?.setleave || `Selamat Tinggal Kak @`;
					if ((jidNormalizedUser(alya.user.lid) == jidNormalizedUser(n)) || (jidNormalizedUser(alya.user.id) == jidNormalizedUser(n))) {
				...
				if (messageText && alya.public) {
					await alya.sendMessage(id, {
...
async function LoadDataBase(alya, m) {
	try {
		...
		const defaultSetBot = {
			...
			multiprefix: true,
			privateonly: false,
			author: global.author || 'AlyaBot',
			autobackup: false,
			botname: global.botname || 'Alya Bot',
			packname: global.packname || '©Stefansk',
...
async function MessagesUpsert(alya, message, store) {
	try {
		...
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) store.groupMetadata ??= await alya.groupFetchAllParticipating().catch(e => ({}));
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		const m = await Serialize(alya, msg, store)
		require('../alya')(alya, m, msg, store);
		if (db?.set?.[botNumber]?.readsw && msg.key.remoteJid === 'status@broadcast') {
			await alya.readMessages([msg.key]);
			if (/protocolMessage/i.test(type)) await alya.sendFromOwner(global.db?.set?.[botNumber]?.owner?.map(x => x.id) || global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
			if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
				...
				await alya.sendFromOwner(global.db?.set?.[botNumber]?.owner?.map(x => x.id) || global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
			}
		}
	} catch (e) {
		throw e;
...
async function Solving(alya, store) {
	alya.serializeM = (m) => MessagesUpsert(alya, m, store)
	...
	alya.getName = (jid, withoutContact  = false) => {
		const id = alya.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || (store.groupMetadata[id] ? store.groupMetadata[id] : (store.groupMetadata[id] = alya.groupMetadata(id))) || {};
	...
	alya.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		...
		for (let i of kon) {
			list.push({
				...
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await alya.getName(i + '@s.whatsapp.net')}\nFN:${await alya.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await alya.getName(i + '@s.whatsapp.net')}\nFN:${await alya.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/alya_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		alya.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	
	alya.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await alya.query({
			tag: 'iq',
			attrs: {
	...
	alya.extractGroupMetadata = (result) => {
	...
	alya.groupMetadata = async (jid) => {
		...
		return alya.extractGroupMetadata(result);
	};
	
	alya.groupFetchAllParticipating = async () => {
		const result = await alya.query({ tag: 'iq', attrs: { to: '@g.us', xmlns: 'w:g2', type: 'get' }, content: [{ tag: 'participating', attrs: {}, content: [{ tag: 'participants', attrs: {}}, { tag: 'description', attrs: {}}]}]});
		const data = {};
		const groupsChild = getBinaryNodeChild(result, 'groups');
		if (groupsChild) {
			const groups = getBinaryNodeChildren(groupsChild, 'group');
			for (const groupNode of groups) {
				const meta = alya.extractGroupMetadata({
					tag: 'result',
					attrs: {},
					content: [groupNode]
				});
		...
		alya.ev.emit('groups.update', Object.values(data));
		return data;
	}

	alya.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return alya.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	}
	...
	alya.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return alya.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, quotedOptions);
			} else if (mime && mime === 'application/pdf') {
				return alya.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('image')) {
				return alya.sendMessage(jid, { image: res.data, caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('video')) {
				return alya.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, quotedOptions);
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return alya.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('audio')) {
				return alya.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, quotedOptions);
			}
		}
	...
	alya.sendGroupInvite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
		...
		const invite = await alya.relayMessage(participant, message.message, { messageId: message.key.id })
		return invite
	}
	
	alya.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await alya.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
	}
	
	alya.sendText = async (jid, text, quoted, options = {}) => alya.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	
	alya.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return alya.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	...
	alya.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		const buffer = await alya.downloadMediaMessage(message);
	...
	alya.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: alya.user.id, quoted: m.quoted && m.quoted.fakeObj, ephemeralExpiration: m.expiration || m?.metadata?.ephemeralDuration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
		apb.key = m.key
		apb.key.id = [...Array(32)].map(() => '0123456789ABCDEF'[Math.floor(Math.random() * 16)]).join('');
		apb.key.fromMe = areJidsSameUser(m.sender, alya.user.id);
		if (m.isGroup) apb.participant = m.sender;
		alya.ev.emit('messages.upsert', {
			...m,
	...
	alya.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await alya.getFile(path, true);
		...
		if (isWebpSticker) {
			pathFile = await writeExif(data, {
				...
				author: options.author || db?.set?.[botNumber]?.author || 'AlyaChan',
		...
		let anu = await alya.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0, ...options });
		await fs.unlinkSync(pathFile);
		return anu;
	}
	
	alya.sendAlbumMessage = async (jid, content = {}, options = {}) => {
		...
		await alya.relayMessage(jid, medias.message, { messageId: medias.key.id });
		for (const media of album) {
			const msg = await generateWAMessage(jid, { ...others, ...media }, { upload: alya.waUploadToServer });
			...
			await alya.relayMessage(jid, msg.message, { messageId: msg.key.id });
		}
		return medias;
	}
	
	alya.sendListMsg = async (jid, content = {}, options = {}) => {
		...
		const hasil = await alya.relayMessage(msg.key.remoteJid, msg.message, {
	...
	alya.sendButtonMsg = async (jid, content = {}, options = {}) => {
	...
	alya.newsletterMsg = async (key, content = {}, timeout = 5000) => {
		...
		if (react) {
			...
			const msg = await generateWAMessageContent(media, { upload: alya.waUploadToServer });
			const anu = await alya.query({
				tag: 'message',
			...
			const _query = await alya.query({
				...
				content: [{
					tag: 'query',
					attrs: {
						query_id: type == 'FOLLOW' ? '120363144038483540' : type == 'UNFOLLOW' ? '120363144038483540' : type == 'CREATE' ? '120363144038483540' : type == 'DELETE' ? '120363144038483540' : '120363144038483540'
					},
	...
	alya.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: alya.waUploadToServer });
			return imageMessage;
		}
		...
		const hasil = await alya.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	if (alya.user && alya.user.id) {
		const botNumber = alya.decodeJid(alya.user.id);
		if (global.db?.set[botNumber]) {
			alya.public = global.db.set[botNumber].public
		} else alya.public = true
	} else alya.public = true
...
async function Serialize(alya, msg, store) {
	const botLid = alya.decodeJid(alya.user.lid);
	const botNumber = alya.decodeJid(alya.user.id);
	const m = { ...msg };
	if (!m) return m
	if (m.key) {
		...
		if (!m.isGroup && m.chat.endsWith('@lid')) m.chat = alya.findJidByLid(m.chat, store) || m.chat;
		m.sender = alya.decodeJid(m.fromMe && alya.user.id || m.key.participant || m.chat || '')
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await alya.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = await alya.groupMetadata(m.chat).catch(e => ({})))
			if (!metadata) {
				metadata = await alya.groupMetadata(m.chat).catch(e => ({}))
			...
			if (metadata.addressingMode === 'lid') {
				...
				store.contacts[m.sender] = { ...store.contacts[m.sender], id: m.sender, lid: m.fromMe && alya.user.lid || participant?.lid || m.sender, name: m.pushName };
			}
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = alya.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === alya.decodeJid(alya.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, alya)
				return await Serialize(alya, q, store)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(alya.decodeJid(m.msg?.contextInfo?.participant), alya.decodeJid(alya?.user?.id)),
			...
			m.quoted.download = () => alya.downloadMediaMessage(m.quoted)
			m.quoted.delete = () => {
				alya.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.download = () => alya.downloadMediaMessage(m)
	
	m.copy = () => Serialize(alya, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
	
	m.react = (u) => alya.sendMessage(m.chat, { react: { text: u, key: m.key }})
	...
	m.reply = async (content, options = {}) => {
		...
		if (typeof content === 'object') {
			return alya.sendMessage(chat, content, { ...options, quoted, ephemeralExpiration })
			...
			try {
				if (/^https?:\/\//.test(content)) {
					...
					if (/gif|image|video|audio|pdf|stream/i.test(mime)) {
						return alya.sendMedia(chat, data.data, '', caption, quoted, content)
					} else {
						return alya.sendMessage(chat, {
  text: content,
  contextInfo: {
    ...
    businessMessageForwardInfo: {
      businessOwnerJid: alya.user.jid // asumsi 'alya' adalah instance bot
    },
    externalAdReply: {
      title: `${author} ( alya )`,
      body: "Alya",
      thumbnailUrl: "https://raw.githubusercontent.com/NdikzDatabase/Database/main/Database/1754250850650-6rln8u.jpg",
      sourceUrl: "https://AlyaChan.com",
      mediaType: 1,
      renderLargerThumbnail: false
    },
    ...(options.contextInfo || {})
